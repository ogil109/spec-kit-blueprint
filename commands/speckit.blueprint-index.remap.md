...
3. Run the oracle's `check` to get the stale/dangling sections:
   `bash .specify/extensions/blueprint-index/scripts/bash/blueprint-state.sh check` (or the
...
4. **Refresh the baseline.** Run the oracle's restamp for the area so the marker
   ...
   `bash .specify/extensions/blueprint-index/scripts/bash/blueprint-state.sh restamp --path src/<area>`
